...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   outlog << "sending header " << header << endl;
   send_packet (client_sock, &header, sizeof header);
   send_packet (client_sock, filecontents.c_str(), filecontents.size());
   outlog << "sent " << filename.size() << " bytes" << endl;
}

void reply_put(accepted_socket& client_sock, cix_header& header){
      string filename = header.filename;
      if(fileExists(filename)){
      outlog << "put: file already exists: "
      << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
...
void reply_rm (accepted_socket& client_sock, cix_header& header){
   string filename = header.filename;
   if(!fileExists(filename)){
      outlog << "rm: ifstream failed: " << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
      return;
   ...
   if (status == -1) {
      outlog << "rm: unlink failed: " << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
      return;
   }
   outlog << "rm: exit " << (status >> 8)
   << " signal " << (status & 0x7F)
   << " core " << (status >> 7 & 1) << endl;
   header.command = cix_command::ACK;
   header.nbytes = 0;
