...
//
// Main function.
// Note that underscores are negative signs, - reserved for subtraction
int main (int argc, char** argv) {
   exec::execname (argv[0]); // Initialize execname, name of executable
   scan_options (argc, argv);
   bigint_stack operand_stack; // Declare major level variables
   scanner input; // Module to read input as tokens
   try {
      for (;;) {
         try {
      ...
      //Catching nested exceptions from other classes.
   }
   return exec::status();
}
